...
public class ConsignmentManagementJFrame extends javax.swing.JFrame {
    ...
    public ConsignmentManagementJFrame() {
        ...
        // Load data
        loadProductDataFromFile("/Users/tranmanhdat/NetBeansProjects/ProductsManagementGCD210057/productData.txt");
    }

    private void loadProductDataFromFile(String filePath) {
        tableModel.setRowCount(0);
        itemList.clear(); // Xóa hết dữ liệu cũ trong itemList
        try {
            ...
            while ((line = bufferedReader.readLine()) != null) {
                ...
                if (data.length >= 7) {
                    ...
                    String consignor = data[6];

                    Item newItem = new Item(productID, productName, times, quantity, getCategoryByName(category), consignor);
                    itemList.add(newItem); // Thêm item vào itemList
            ...
            JOptionPane.showMessageDialog(this, "Error loading product data from file.", "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void saveTableToTxtFileBtnActionPerformed(java.awt.event.ActionEvent evt) {
        try {
            ...
            for (Item item : itemList) {
                String productData = String.format("%s,%s,%d,%d,%.2f,%s,%s",
                        item.getProductID(), item.getProductName(), item.getQuantity(),
                        item.getTimes(), item.getCategory().getDefaultPrice() * item.getQuantity() * item.getTimes(),
                        item.getCategory().getCategoryName(), item.getConsignor());
    ...
    private void editBtnActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        if (selectedRow != -1) {
            ...
            Item updatedItem = new Item(productID, productName, times, quantity, category, consignor);
            itemList.set(selectedRow, updatedItem);
            ...
            tableModel.setValueAt(times, selectedRow, 3);
            tableModel.setValueAt(price, selectedRow, 4);
            tableModel.setValueAt(category.getCategoryName(), selectedRow, 5);
            tableModel.setValueAt(consignor, selectedRow, 6);
